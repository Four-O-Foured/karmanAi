...
const ChatInterface = ({ onToggleSidebar, chatId, socket }) => {
    ...
    // Mutation to auto-create a chat when starting from an empty state
    const createChatMutation = useMutation({
        mutationFn: async (title) => {
            const response = await api.post('/chat', { title });
            return response.data;
        },
        onSuccess: (newChat, originalMessage) => {
            queryClient.invalidateQueries(['chats']);

            // Optmistic UI update so the message appears instantly in the new chat
            const userMsg = {
                _id: Date.now(),
                role: 'user',
            ...
            queryClient.setQueryData(['messages', newChat._id], [userMsg]);

            // Move user to the new chat URL silently
            navigate({ to: '/chat/$chatId', params: { chatId: newChat._id } });

            // Ensure the AI actually receives the prompt now that the chat container exists
        ...
        onError: (err) => {
            setIsThinking(false);
            console.error("Failed to initialize tracking for new mission:", err);
        }
    });
    ...
    const onSubmit = (data) => {
        ...
        // Auto-create chat if we are in the empty state (no active chatId)
        if (!chatId) {
            createChatMutation.mutate(content);
            return;
        }

        // Optimistic update for standard messages
        const userMsg = {
            _id: Date.now(),
            role: 'user',
            content,
        ...
        <main className="flex-1 flex flex-col h-full relative bg-karman-surface rounded-xl md:rounded-2xl border border-karman-border overflow-hidden min-w-0">
            ...
            >
                {messages.length === 0 && !isThinking ? (
                    ...
                    >
                        <div className="w-16 h-16 rounded-full karman-gradient flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(36,59,85,0.3)]">
                            <Rocket className="w-8 h-8 text-white" />
                        </div>
                        <h3 className="text-2xl font-bold text-white font-heading mb-2 tracking-tight">
                            Welcome to Karman.
                        </h3>
                        <p className="text-karman-muted max-w-md leading-relaxed text-sm">
                            Mission control is online. Initialize a new mission below or ask me anything to get started.
                        </p>
                    </motion.div>
                ) : (
